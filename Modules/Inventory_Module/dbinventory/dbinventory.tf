data "template_file" "dbinventory" {
  ...
  vars = {
    oracledbs    = "${join("", [for server in var.oracleservers : join("", [contains(regexall(".*db.*|.*master$", server.display_name), server.display_name) ? "${server.display_name} ansible_host=${server.public_ip} ansible_connection=ssh ansible_user=ubuntu\n" : ""])])}"
    azuredbs     = "${join("", [for server in var.azureservers : join("", [contains(regexall(".*db.*|.*master$", server.display_name), server.display_name) ? "${server.display_name} ansible_host=${server.public_ip} ansible_connection=ssh ansible_user=ubuntu\n" : ""])])}"
